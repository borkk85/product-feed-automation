...
class PFA_Queue_Manager
{
    ...
    try {
        ...
        $this->log_message("Retrieved product {$product_id} from queue. {$remaining} items remaining.");
        
        // IMPORTANT: Update both storage methods to keep them in sync
        $set_result = set_transient($this->queue_key, $queue, DAY_IN_SECONDS);
        update_option($this->queue_key . '_backup', $queue);
        
        if (!$set_result) {
            $this->log_message("WARNING: Failed to update transient queue. Using option backup.");
        }
